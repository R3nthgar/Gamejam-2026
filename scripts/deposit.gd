...
func _ready() -> void:
	super()
	for potion_instance in shop_handler.potions:
		var potion_name=potion_instance[0]
		var potion_metadata=potion_instance[1]
		var new_potion=shop_handler.potion_names[potion_name].instantiate()
	...
	for collectible_instance in shop_handler.ingredients:
		for collectible_num in shop_handler.ingredients[collectible_instance]:
...
func _on_container_area_1_body_entered(body: Node2D) -> void:
	if not body.get_meta("start_inside"):
		if body.get_meta("color"):
			...
			shop_handler.potions.append([body.get_script().get_global_name(), metadata])
		else:
...
func _on_container_area_2_body_exited(body: Node2D) -> void:
	if body.get_meta("color"):
		if shop_handler.resetting:
			instanced_potions.erase(body)
		else:
			var index=instanced_potions.find(body)
			instanced_potions.remove_at(index)
			shop_handler.potions.remove_at(index)
	else:
		if body.get_meta("collectible") in shop_handler.ingredients and not shop_handler.resetting:
			shop_handler.ingredients[body.get_meta("collectible")]-=1
