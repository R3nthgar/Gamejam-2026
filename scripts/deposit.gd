...
@onready var player: Player = %Player if (true if not OS.is_debug_build() else not Engine.is_editor_hint()) else null
	...
	if (true if not OS.is_debug_build() else not Engine.is_editor_hint()):
		...
		for collectible_instance in global_handler.ingredients:
			for collectible_num in global_handler.ingredients[collectible_instance]:
				...
				new_collectible.collectible=collectible_instance
...
func add_to_deposit(body: Node2D):
	if body is Potion:
		...
		if body.collectible in global_handler.ingredients:
			global_handler.ingredients[body.collectible]+=1
		else:
			global_handler.ingredients[body.collectible]=1
...
func _on_container_area_2_body_exited(body: Node2D) -> void:
	if body is Potion:
		...
		if body.collectible in global_handler.ingredients and body.held and not global_handler.resetting:
			global_handler.ingredients[body.collectible]-=1
